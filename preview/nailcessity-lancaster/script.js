// Micro-interactions for the form and scroll reveal
        document.querySelectorAll('input, textarea, select').forEach(el => {
            el.addEventListener('focus', () => {
                el.parentElement.querySelector('label').style.opacity = '1';
                el.parentElement.querySelector('label').style.color = '#fff8f4';
            });
            el.addEventListener('blur', () => {
                if(!el.value) {
                    el.parentElement.querySelector('label').style.opacity = '0.6';
                }
            });
        });
