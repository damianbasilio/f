        ...
        form.addEventListener('submit', (e) => {
            ...
            requiredFields.forEach(field => {
                ...
                if (!field.value.trim()) {
                    ...
                    if (errorSpan) errorSpan.classList.remove('opacity-0');
                    isValid = false;
                } else {
            ...
            if (isValid) {
                // Animate out inputs
                const children = form.querySelectorAll('div, button');
                children.forEach((child, index) => {
                    child.style.transition = `opacity 0.3s ease ${index * 0.05}s`;
                    child.style.opacity = '0';
                    child.style.pointerEvents = 'none';
                });

                setTimeout(() => {
                    form.querySelectorAll('.grid, .relative, .flex').forEach(el => el.classList.add('hidden'));
