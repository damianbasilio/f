// Micro-interaction for hero button
        document.querySelector('#velocity-hero button').addEventListener('mousedown', function() {
            this.classList.add('scale-95');
        });
        document.querySelector('#velocity-hero button').addEventListener('mouseup', function() {
            this.classList.remove('scale-95');
        });

        // Simple scroll parallax effect for speed lines
        window.addEventListener('scroll', function() {
            const scroll = window.pageYOffset;
            const heroImage = document.querySelector('#velocity-hero img');
            if(heroImage) {
                heroImage.style.transform = `translateY(${scroll * 0.1}px) scale(1.1)`;
            }
        });
