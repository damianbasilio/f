...
import { buildFullSingleAttemptPrompt, loadBriefAndBrand, readPromptBody, buildRebuildVariationNote } from "./stitch-build-prompt.mjs";
import { Stitch } from "@google/stitch-sdk";
import {
  isStitchCreditError,
  findNextKeyIndex,
  logCreditFailover,
  throwIfStitchKeyFailover,
} from "./stitch-credit-error.mjs";
...
async function ensureBrandExtract(slug, brief) {
  ...
  if (!siteUrl) {
    console.warn("No Live URL in brief — skipping source extract");
    return;
  }
  if (fs.existsSync(brandPath)) {
    const existing = JSON.parse(fs.readFileSync(brandPath, "utf8"));
    if (existing.sourceUrl === siteUrl) {
      console.log("Source metadata up to date.");
      return;
    }
  }
  console.log("Recording Facebook source metadata…");
  ...
  if (brand.facebookOnly) {
    ...
    console.log(`Brand colors: ${brand.colors.slice(0, 6).join(", ")}`);
  }
}
  ...
  try {
    if (provided?.stitch && provided?.callTool) {
      return await runBuildSlug(slug, {
        ...
        rebuild: Boolean(provided.rebuild),
      });
    }
    ...
    let lastError;
    let keyIndex = allocateStitchKeyIndex(keys.length, exhausted);
    ...
    while (keyIndex !== null) {
      ...
      try {
        ...
        if (isStitchCreditError(err) && keys.length > 1) {
          ...
          keyIndex = findNextKeyIndex(exhausted, keys.length, keyIndex + 1);
          continue;
        }
        throw err;
      } finally {
...
async function runBuildSlug(slug, { stitch, callTool, ownsClient, close, rebuild = false }) {
  try {
    ...
    const attemptPrompt = buildFullSingleAttemptPrompt({ slug, businessName, contact, rebuild });
    ...
    if (attemptA.ok) {
      ...
      throwIfStitchKeyFailover(attemptA.error);
    ...
    if (!screenHasHtml(screen)) {
      if (!designMd?.trim()) {
        ...
        const recoveryPrompt = readPromptBody(slug) + (rebuild ? buildRebuildVariationNote() : "");
        ...
        if (recovery.ok) {
          ...
          throwIfStitchKeyFailover(recovery.error);
        }
      } else {
      ...
      if (!screenHasHtml(screen)) {
        ...
        const sitePrompt = buildSiteGeneratePrompt({
          ...
          ...contact,
          designMd,
        }) + (rebuild ? buildRebuildVariationNote() : "");
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
          ...
          throwIfStitchKeyFailover(gen.error);
          if (attempt === retries) {
