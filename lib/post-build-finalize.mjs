...
import { repairSiteStructure } from "./site-structure-fix.mjs";
import { applySiteContentFix } from "./site-content-fix.mjs";
...
export async function finalizePostBuild(slug) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const structure = repairSiteStructure(slug);
    if (structure.fixed) console.log(`  ✓ structure-fix — ${structure.changes.join("; ")}`);

    const content = applySiteContentFix(slug);
    if (content.fixed) console.log(`  ✓ content-fix — ${content.changes.join("; ")}`);
