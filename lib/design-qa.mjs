...
export function runDesignQa(slug) {
  ...
  const fixes = [];
  const findings = { emil: [], taste: [], audit: [], harden: [], polish: [] };
  ...
  if (!html.includes("mockup-banner")) findings.audit.push("FAIL: missing mockup banner");
  else findings.audit.push("Banner: present");
  ...
  const blockers = Object.values(findings).flatMap((items) => items.filter((f) => f.startsWith("FAIL:")));
  const ok = blockers.length === 0;

  fs.mkdirSync(reportDir, { recursive: true });
  const reportPath = path.join(reportDir, "report.md");
  fs.writeFileSync(reportPath, buildReport(slug, findings, fixes, ok), "utf8");
...
function buildReport(slug, findings, fixes, ok) {
  const sections = ["emil", "taste", "audit", "harden", "polish"];
