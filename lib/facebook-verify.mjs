...
import { isWebsiteLive } from "./website-live-check.mjs";
...
export async function verifyAndScrapeFacebook(facebookUrl, lead, slug, assetsDir) {
  ...
  try {
    ...
    const postTimestamps = extractPostTimestampsFromHtml(html);
    ...
    if (fbWebsiteClass === "owned") {
      const liveCheck = await isWebsiteLive(fbWebsiteUrl);
      result.fbWebsiteLive = liveCheck.live;
      if (liveCheck.live) {
        result.status = "skip";
        result.reason = "fb_website_owned_domain";
        return result;
      }
      result.reason = "fb_website_owned_but_dead";
    } else if (fbWebsiteClass === "directory") {
