 ...
 */
import { isUsefulCaption } from "./fb-caption-sanitize.mjs";
 ...
 */
export function extractPostTimestampsFromHtml(html) {
  if (!html) return [];
  const secs = new Set();

  for (const m of html.matchAll(
    /"(?:creation_time|publish_time|story_publish_time|timestamp|created_time)":(\d{10,13})/g
  )) {
    let ts = Number(m[1]);
    if (ts > 1e12) ts = Math.floor(ts / 1000);
    if (ts > 946684800 && ts < 4102444800) secs.add(ts);
  }

  for (const m of html.matchAll(/"timestamp":\{"timestamp":(\d{10,13})\}/g)) {
    let ts = Number(m[1]);
    if (ts > 1e12) ts = Math.floor(ts / 1000);
    if (ts > 946684800 && ts < 4102444800) secs.add(ts);
  }
...
export function pickBestIntro(pageData, about, postCaptions) {
  const candidates = [
    ...
    pageData.title,
    ...postCaptions.filter((c) => c.length >= 35 && !isProfileFieldNoise(c) && isUsefulCaption(c)),
  ].filter(Boolean);
...
export function buildProfileSummary(pageData, about, postCaptions) {
  ...
  if (postCaptions.length) {
    const posts = postCaptions.filter((c) => !isProfileFieldNoise(c) && isUsefulCaption(c));
    if (posts.length) {
