...
const MAX_VALIDATION_REWRITES = 3;

export { MAX_VALIDATION_REWRITES };

/**
 * Groq rewrite prompt when draft validation fails.
 */
export function buildGroqRewriteUserPrompt(ctx, { subject, body, errors }) {
  return `${buildGroqUserPrompt(ctx)}

## REWRITE REQUIRED
Your previous draft failed automated validation. Fix every issue below and return corrected JSON only.
...
Return valid JSON with a corrected subject and body.`;
}
