...
const CREDIT_MESSAGE_PATTERNS = [
  ...
  /\bcredit balance\b/i,
  /\bcredits? (?:for|per) (?:the )?day\b/i,
  /\bdaily (?:credit|quota|limit)\b/i,
  /\bquota exceeded\b/i,
  /\bexceeded.*\bquota\b/i,
  /\bexceeded your current quota\b/i,
  /\brate limit exceeded\b/i,
  /\btoo many requests\b/i,
  // Google Stitch SDK: "Resource has been exhausted (e.g. check quota)."
  /\bresource\b[\s\S]{0,48}\bexhausted\b/i,
  /\bresource[\s_-]*exhausted\b/i,
  /Tool Call Failed\b[\s\S]*\b(?:exhausted|quota)\b/i,
...
/**
 * Throw quota/credit errors so callers rotate API keys instead of retrying the same key.
 * @param {unknown} error
 */
export function throwIfStitchKeyFailover(error) {
  if (error && isStitchCreditError(error)) {
