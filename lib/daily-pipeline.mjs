...
/** CLI / PM2 entry (see scripts/outreach-pipeline.mjs). */
export function runDailyPipelineCli(argv = process.argv.slice(2)) {
  if (argv.includes("--once-morning")) {
    runMorningPipeline()
      .then(() => process.exit(0))
      .catch(() => process.exit(1));
    return;
  }
  if (argv.includes("--once-eight-am")) {
    runEightAmJobs()
      .then(() => process.exit(0))
      .catch(() => process.exit(1));
    return;
