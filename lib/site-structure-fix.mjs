...
function fixHeroCredibilityLayout(doc) {
  ...
  const contentWrap = hero.querySelector(":scope > div.relative");
  if (contentWrap) {
    let wrapCls = contentWrap.getAttribute("class") || "";
    if (!/\bpt-\d+\b/.test(wrapCls)) {
      wrapCls = `${wrapCls} pt-8 md:pt-0`.replace(/\s+/g, " ").trim();
      contentWrap.setAttribute("class", wrapCls);
    }
  }
