...
import { craftSafeCaptions } from "./design-craft.mjs";
...
export async function enrichLead(slug, lead, verifyResult) {
  ...
  const postCaptions = craftSafeCaptions(verifyResult.postCaptions || []);
  const captionSnippet =
    postCaptions.find((c) => c.length >= 25 && c.length <= 120 && !isRegistrationListingNoise(c)) ||
  ...
  const promptInput = {
    ...
    layoutArchetype: profile.layoutArchetype || profile.layoutFamily,
...
- **Tone:** ${profile.mood}
- **Layout archetype:** ${profile.layoutArchetype || profile.layoutFamily}
...
function composeHeroH1(name, city, profile, captionSnippet) {
  ...
  if (captionSnippet.length > 15 && captionSnippet.length < 70 && !isRegistrationListingNoise(captionSnippet)) {
