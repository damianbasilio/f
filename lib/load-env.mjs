...
/** Concurrent preflight workers when Stitch is disabled (Facebook verify uses Puppeteer). */
export function getPipelineConcurrency(slugCount) {
  const env = Number(process.env.PIPELINE_CONCURRENCY);
  const cap = Number.isFinite(env) && env > 0 ? env : 3;
  return Math.min(slugCount, cap);
}
