...
import {
  SYSTEM_PROMPT,
  buildGroqUserPrompt,
  buildGroqRewriteUserPrompt,
  GROQ_BODY_WORD_MAX,
  MAX_VALIDATION_REWRITES,
} from "./outreach-groq-prompt.mjs";
import { OUTREACH_SIGNATURE } from "./parse-outreach.mjs";
import { validateOutreachDraft } from "./outreach-draft-validate.mjs";
...
export async function buildFbOutreachCopy(slug, batchIndex = 0) {
  ...
  let copy;
  if (cfg.enabled) {
    copy = await buildFbOutreachCopyGroq(slug, batchIndex, cfg);
  } else {
    console.warn(`  ○ GROQ_API_KEY not set — using template outreach for ${slug}`);
    copy = buildFbOutreachCopyTemplate(slug, batchIndex);
    ...
    if (!validation.ok) {
      throw new Error(`Outreach validation failed: ${validation.errors.join("; ")}`);
    }
  }
  return copy;
}
...
async function buildFbOutreachCopyGroq(slug, batchIndex, cfg) {
  ...
  let userPrompt = buildGroqUserPrompt(ctx);
  let lastDraft = { subject: "", body: "" };
  let lastValidationErrors = [];

  let usage;
  let lastError;

  for (let rewrite = 0; rewrite < MAX_VALIDATION_REWRITES; rewrite++) {
    for (let attempt = 0; attempt <= 2; attempt++) {
      const temperature = rewrite === 0 && attempt === 0 ? 0.55 : 0.35;
      try {
        ...
        const rawBody = stripEmDashes(String(parsed.body || ""));
        const trimmedBody = enforceGroqBodyWordLimit(rawBody, GROQ_BODY_WORD_MAX);
        const copy = groqCopyFromParsed({ ...parsed, body: trimmedBody }, ctx);
        lastDraft = { subject: copy.subject, body: copy.body };
        ...
        if (!validation.ok) {
          ...
          console.warn(
            `  Outreach validation for ${slug} — rewrite (${rewrite + 1}/${MAX_VALIDATION_REWRITES}): ${validation.errors.join("; ")}`
          );
          userPrompt = buildGroqRewriteUserPrompt(ctx, {
            subject: copy.subject,
            body: copy.body,
          ...
          JSON.stringify(
            {
              ...
              usage,
              raw: { ...parsed, body: trimmedBody },
              bodyWordCount: countBodyWordsExcludingUrl(trimmedBody),
              trimmedFrom: countBodyWordsExcludingUrl(rawBody),
              validationRewrites: rewrite,
            },
            null,
            2
          ) + "\n",
    ...
    if (rewrite < MAX_VALIDATION_REWRITES - 1 && lastValidationErrors.length) {
      continue;
    }
    break;
  }
  ...
  if (lastValidationErrors.length) {
    throw new Error(`Outreach validation failed: ${lastValidationErrors.join("; ")}`);
  }

  throw lastError || new Error(`Groq outreach failed for ${slug}`);
}

/** Groq JSON → outreach copy (body already word-limited). Signature added in writeGroqOutreachMd. */
...
/** Word count for Groq body; preview URL line is excluded (matches prompt). */
function countBodyWordsExcludingUrl(body) {
  return wordCount(
    String(body || "")
      .split(/\n\n+/)
      .map((p) => p.trim())
      .filter((p) => p && !isUrlLine(p))
      .join(" ")
  );
}

/** Trim prose paragraphs until body is within max (URL lines preserved). */
function enforceGroqBodyWordLimit(body, max = GROQ_BODY_WORD_MAX) {
  ...
  while (countBodyWordsExcludingUrl(blocks.join("\n\n")) > max) {
    let longestIdx = -1;
    let longest = 0;
    for (let i = 0; i < blocks.length; i++) {
      if (isUrlLine(blocks[i])) continue;
    ...
    if (sentences.length > 1) {
      blocks[longestIdx] = sentences.slice(0, -1).join(" ");
      continue;
    }

    const words = block.split(/\s+/);
    if (words.length <= 4) break;
    blocks[longestIdx] = words.slice(0, -3).join(" ").replace(/[,;]\s*$/, "") + ".";
  }
